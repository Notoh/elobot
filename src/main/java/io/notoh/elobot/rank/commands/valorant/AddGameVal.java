...
public class AddGameVal extends Command {
    ...
    public void run(Message msg) {
        ...
        if(args.length < 22) {
            msg.getChannel().sendMessage("Correct usage: -addgameval <names> <roundswinner> <roundsloser> <corresponding_performances>").queue();
            return;
        }
        ...
        int winnerAvg = 0;
        for(int i = 0; i < 5; i++) {
            winnerAvg += database.getValPlayers().get(names[i]).getRating();
        }
        winnerAvg /= 5;
        int loserAvg = 0;
        for(int i = 5; i < 10; i++) {
            loserAvg += database.getValPlayers().get(names[i]).getRating();
        }
        loserAvg /= 5;
